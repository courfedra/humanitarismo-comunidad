    ...
    <nav className="fixed w-full z-50 transition-all duration-300 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`flex items-center justify-between transition-all duration-300 ${
          scrolled ? 'py-2 md:py-3' : 'py-2 md:py-6'
        }`}>
          <div className="flex items-center">
            ...
            {/* Desktop Navigation */}            <div className="hidden sm:flex items-center space-x-14 ml-14">
          ...
          <div className="flex items-center gap-4">
            {/* Donate Button - Both Mobile & Desktop */}
            <Link 
              href="https://donaronline.org/humanitarismo-comunidad/ayudanos-a-llegar-a-las-comunidades" target='_blank'
              className="px-6 py-2 bg-[#5E17EB] text-white rounded-full hover:bg-[#4c12c4] transition-colors duration-300"
            >
              Quiero donar
            </Link>
...
function NavLink({ href, children, onClick, disabled }) {
  ...
  return (
    ...
    >
      ...
      <span 
        className={`absolute -bottom-0.5 left-0 w-full h-[2px] bg-[#F6A717] transform origin-left transition-all duration-300 ease-in-out ${
          isActive 
            ? 'scale-x-100' 
            : 'scale-x-0'
        }`}
